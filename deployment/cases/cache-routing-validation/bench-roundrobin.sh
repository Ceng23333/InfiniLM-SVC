#!/usr/bin/env bash
# Benchmark script for round-robin configuration
# Both instances running, no prompt_cache_key (default routing)
...
if [ -z "${TOKENIZER_DIR}" ] || [ ! -d "${TOKENIZER_DIR}" ]; then
  ...
  echo "    ./bench-roundrobin.sh 172.22.162.18"
  echo ""
...
# Activate conda environment if available (ensure it's activated)
if command -v conda &> /dev/null; then
  eval "$(conda shell.bash hook)"
  conda activate "${CONDA_ENV_NAME}" 2>/dev/null || {
    echo "Warning: Could not activate conda environment '${CONDA_ENV_NAME}'"
...
# Run vLLM bench without prompt_cache_key (round-robin routing)
# Use -u for unbuffered output to see progress in real-time
echo "Executing benchmark (full output logged to: ${SCRIPT_DIR}/bench-roundrobin.log)..."
echo ""

# Run benchmark with output to both log file and stdout
# Use a temporary file to ensure all output is captured
TEMP_LOG="${SCRIPT_DIR}/bench-roundrobin.tmp.log"

# Call the benchmark using helper script
...
mv "${TEMP_LOG}" "${SCRIPT_DIR}/bench-roundrobin.log" 2>/dev/null || true
...
if [ ${BENCHMARK_EXIT_CODE} -eq 0 ]; then
  ...
  if [ -d "${SCRIPT_DIR}/results" ] && [ "$(ls -A ${SCRIPT_DIR}/results 2>/dev/null)" ]; then
    ...
    RESULT_FILES=$(find "${SCRIPT_DIR}/results" -name "round-robin-*.json" -type f -mmin -5 2>/dev/null | sort -r)
    if [ -n "${RESULT_FILES}" ]; then
      ...
      if [ -n "${LATEST_RESULT}" ]; then
        ...
        echo "Check the log file for details: ${SCRIPT_DIR}/bench-roundrobin.log"
        echo ""
  ...
  tail -30 "${SCRIPT_DIR}/bench-roundrobin.log" 2>/dev/null || echo "  (log file not found)"
  exit 1
fi
