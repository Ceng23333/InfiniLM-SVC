#!/usr/bin/env bash
# Benchmark script for size-based routing configuration
# Both instances running (paged and static cache), with size-based routing
...
MAX_CONCURRENCY="${MAX_CONCURRENCY:-4}"  # Limit concurrent requests to reduce load on static cache

# Large context configuration (for size-based routing)
...
CONTEXT_LEN="${CONTEXT_LEN:-1024}"                # Context length per message
NEW_MSG_LEN="${NEW_MSG_LEN:-128}"                 # New message length
LARGE_CONTEXT_LEN="${LARGE_CONTEXT_LEN:-2048}"    # Length of large initial context
DATASET_FILE="${DATASET_FILE:-${SCRIPT_DIR}/large_context.jsonl}"
NUM_REQUESTS=$((NUM_CONVERSATIONS * MESSAGES_PER_CONV))

# Routing threshold (default: 50KB)
ROUTING_THRESHOLD="${CACHE_TYPE_ROUTING_THRESHOLD:-51200}"
...
if [ -z "${TOKENIZER_DIR}" ] || [ ! -d "${TOKENIZER_DIR}" ]; then
  ...
  echo "    ./bench-size-based-routing.sh 172.22.162.18"
  echo ""
...
echo "=========================================="
echo "Size-Based Routing Benchmark"
echo "=========================================="
...
echo "Routing Threshold: ${ROUTING_THRESHOLD} bytes"
echo ""
echo "Configuration: Both instances running (paged:8100, static:8200)"
echo "Routing: Size-based (small -> paged, large -> static)"
...
if ! curl -s -f --connect-timeout 3 "http://${REGISTRY_IP}:8201/health" > /dev/null 2>&1; then
  echo "Warning: static-cache-9g_8b_thinking (8201) is not responding"
fi
...
echo "Executing benchmark (full output logged to: ${SCRIPT_DIR}/bench-size-based-routing.log)..."
echo ""

# Run benchmark with output to both log file and stdout
# Use a temporary file to ensure all output is captured
TEMP_LOG="${SCRIPT_DIR}/bench-size-based-routing.tmp.log"
...
mv "${TEMP_LOG}" "${SCRIPT_DIR}/bench-size-based-routing.log" 2>/dev/null || true
...
if [ ${BENCHMARK_EXIT_CODE} -eq 0 ]; then
  ...
  if [ -d "${SCRIPT_DIR}/results" ] && [ "$(ls -A ${SCRIPT_DIR}/results 2>/dev/null)" ]; then
    ...
    RESULT_FILES=$(find "${SCRIPT_DIR}/results" -name "size-based-routing-*.json" -type f -mmin -5 2>/dev/null | sort -r)
    if [ -n "${RESULT_FILES}" ]; then
      ...
      if [ -n "${LATEST_RESULT}" ]; then
        ...
        echo "Check the log file for details: ${SCRIPT_DIR}/bench-size-based-routing.log"
        echo ""
        echo "Last 50 lines of log:"
        tail -50 "${SCRIPT_DIR}/bench-size-based-routing.log" 2>/dev/null || echo "  (log file not found)"
      fi
    fi
  else
    echo "⚠️  Warning: No results files found in ${SCRIPT_DIR}/results/"
  ...
  echo ""
  echo "📊 Size-Based Routing Validation:"
  echo "  - Tests routing based on message body size"
  echo "  - Small requests (≤${ROUTING_THRESHOLD} bytes) route to paged cache (GPU 0)"
  echo "  - Large requests (>${ROUTING_THRESHOLD} bytes) route to static cache (GPU 2)"
  echo "  - Dataset includes mix of small and large contexts"
  echo ""
  echo "  To compare cache types, run:"
  echo "    python ${SCRIPT_DIR}/compare-cache-types.py \\"
  echo "      --auto-detect"
else
  EXIT_CODE=$?
  echo ""
  ...
  tail -30 "${SCRIPT_DIR}/bench-size-based-routing.log" 2>/dev/null || echo "  (log file not found)"
  exit 1
fi
